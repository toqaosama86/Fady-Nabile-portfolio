...
const iconMap: Record<string, any> = {
  Film, Palette, Wand2, Smartphone, Youtube, Megaphone, Sparkles, Camera,
};
...
export const ServicesSection = () => {
  ...
  const services = dbServices.length > 0
    ? dbServices.map((s) => ({ icon: s.icon || "Film", title: s.title, description: s.description }))
    : fallbackServices;
      ...
      <div className="max-w-6xl mx-auto">
        <AnimatedSection>
          <div className="text-center mb-14">
            <p className="text-primary font-heading font-medium tracking-[0.2em] uppercase text-xs mb-4">Services</p>
              ...
              return (
                <AnimatedSection key={service.title} delay={i * 80}>
                  <div className="glass-card rounded-xl p-6 hover:border-primary/30 transition-all duration-500 group h-full">
                    <Icon size={28} className="text-primary mb-4 group-hover:scale-110 transition-transform duration-300" />
